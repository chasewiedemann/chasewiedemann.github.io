...
## Market Failures in Municipal Competition

The geographic structure of the economy can be seen as a self-replicating system. At the highest level, we consider the global economy, where large economic groups like the EU or NAFTA compete. If we zoom in one level, this process repeats. Instead of looking at global competition, we now focus on how countries within these groups compete against each other. This layered structure creates complex incentives and welfare calculations: a country’s success depends not only on its own performance but also on the performance of its entire economic group. According to the First Welfare Theorem, we can rely on competition among countries to provide a Pareto efficient outcome. Relying on the Second Welfare Theorem allows for selection of the most efficient Pareto outcome for the benefit of the whole group. These assumptions hold true only if there are no market inefficiencies.
...
The caveats surrounding market inefficiencies are the main topic of this paper. To clearly see these inefficiencies, we make three key assumptions. First, we assume that there are no market inefficiencies, except potentially at the municipal level, which is the focus of this study. Second, we assume that there are no market inefficiencies in other urban areas outside the one we are studying. Third, we assume that we are close and below the Pareto optimal allocation that would exist without any market inefficiencies. This allows us to consider the effects of interventions a full rather than partial equilibrium. Stated plainly, the assumption allow interventions that can increase welfare in a specific urban area, without negatively impacting other urban areas or the country overall.
...
What is driving this market failure, and why is it not unreasonable to assume that it is not present at higher levels of geographic competition? The key detail is that municipalities compete within a specific geographic space and do not have fixed jurisdictional boundaries. Consider the next higher level of geographic competition between urban areas within a country. The cycle above is not possible, as high quality urban areas are allowed to continually expand their boundaries.    
